...
LORA_PATH="${LORA_PATH:-${SUPPLEMENTARY_ROOT}/checkpoints/response_generator/with_persona/qwen2.5-7b-instruct/2026-05-23-01-05-37/checkpoint-16400}"
INPUT_FILE="${INPUT_FILE:-${SUPPLEMENTARY_ROOT}/data/evocrs/response_generator/target_response/with_persona/response_generation_test.json}"
OUTPUT_DIR="${OUTPUT_DIR:-${SCRIPT_DIR}/output/with_persona/qwen2.5-7b-instruct/checkpoint-16400}"
...
for ((SHARD_IDX = 0; SHARD_IDX < WORLD_SIZE; SHARD_IDX++)); do
  ...
  CUDA_VISIBLE_DEVICES="${GPU_ID}" python response_generator_inference.py \
    ...
    > "logs/response_generator_with_persona_qwen25_shard${SHARD_IDX}.log" 2>&1 &
done
