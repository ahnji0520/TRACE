...
LORA_PATH="${LORA_PATH:-${SUPPLEMENTARY_ROOT}/checkpoints/response_generator/without_persona/llama-3.1-8b-instruct/2026-05-22-18-48-24/checkpoint-16400}"
...
OUTPUT_DIR="${OUTPUT_DIR:-${SCRIPT_DIR}/output/without_persona/llama-3.1-8b-instruct/checkpoint-16400}"
...
for ((SHARD_IDX = 0; SHARD_IDX < WORLD_SIZE; SHARD_IDX++)); do
  ...
  CUDA_VISIBLE_DEVICES="${GPU_ID}" python response_generator_inference.py \
    ...
    > "logs/response_generator_without_persona_shard${SHARD_IDX}.log" 2>&1 &
done
