...
LORA_PATH="${LORA_PATH:-${SUPPLEMENTARY_ROOT}/checkpoints/persona_processor/behavioral_persona_extract/qwen2.5-7b-instruct/2026-05-19-20-16-37/checkpoint-11200}"
OUTPUT_DIR="${OUTPUT_DIR:-${PERSONA_DIR}/output/qwen2.5-7b-instruct/bp}"
...
for ((SHARD_IDX = 0; SHARD_IDX < WORLD_SIZE; SHARD_IDX++)); do
  ...
  CUDA_VISIBLE_DEVICES="${GPU_ID}" python "${PERSONA_DIR}/src/extract_bp.py" \
    "${COMMON_ARGS[@]}" \
    --shard_idx "${SHARD_IDX}" \
    > "logs/bp/log_extract_bp_qwen_shard${SHARD_IDX}.log" 2>&1 &
done

wait

cat "${OUTPUT_DIR}"/extracted_behavior_only.shard*.jsonl \
  > "${OUTPUT_DIR}"/extracted_behavior_only.qwen.all.jsonl
...
if len(ids) != len(set(ids)):
    raise SystemExit("duplicate ids detected in merged qwen behavior output")
PY
