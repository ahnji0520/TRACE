...
pat = re.compile(r"^updated_personas_logs\.qwen\.retry(\d+)\.jsonl$")
rounds = []
for path in out_dir.glob("updated_personas_logs.qwen.retry*.jsonl"):
...
OUTPUT_PATH="${OUTPUT_PATH:-${OUTPUT_DIR}/updated_personas_logs.qwen.${ROUND_TAG}.jsonl}"
LOG_PATH="${LOG_PATH:-${SCRIPT_DIR}/logs/update/log_update_qwen_${ROUND_TAG}.log}"

if [[ -z "${FAILED_INPUT_PATH:-}" ]]; then
  if [[ "${RETRY_ROUND}" -eq 1 ]]; then
    FAILED_GLOB="${FAILED_GLOB:-${OUTPUT_DIR}/failed_update_inputs.shard*.json}"
  else
    FAILED_INPUT_PATH="${OUTPUT_DIR}/updated_personas_logs.qwen.retry${PREV_ROUND}.failed.json"
  fi
fi

if [[ -z "${FAILED_INPUT_PATH:-}" ]]; then
  FAILED_INPUT_PATH="${OUTPUT_DIR}/failed_update_inputs.qwen.${ROUND_TAG}.json"
...
if len(ids) != len(set(ids)):
    raise SystemExit("duplicate ids detected in qwen retry output")
PYCHECK
