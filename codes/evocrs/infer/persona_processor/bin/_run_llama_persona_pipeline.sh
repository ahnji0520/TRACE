...
BP_DIR="${BP_DIR:-${PERSONA_DIR}/output/llama-3.1-8B/bp}"
DP_DIR="${DP_DIR:-${PERSONA_DIR}/output/llama-3.1-8B/dp}"
UPD_DIR="${UPD_DIR:-${PERSONA_DIR}/output/llama-3.1-8B/upd}"
UPDATE_CUDA_VISIBLE_DEVICES="${UPDATE_CUDA_VISIBLE_DEVICES:-${CUDA_VISIBLE_DEVICES:-0,1}}"
...
merge_behavior_final() {
  ...
  python "${PERSONA_DIR}/tools/merge_extraction_retries.py" \
    ...
    --base-name "extracted_behavior_only.llama.all.jsonl" \
    --final-name "extracted_behavior_only.llama.final.jsonl" \
    "${extra_args[@]}"
}
...
merge_dialogue_final() {
  ...
  python "${PERSONA_DIR}/tools/merge_extraction_retries.py" \
    ...
    --kind dialogue \
    --model-family "${MODEL_FAMILY}" \
    --base-name "extracted_dialogue_only.llama.all.jsonl" \
    --final-name "extracted_dialogue_only.llama.final.jsonl" \
    "${extra_args[@]}"
}

if [[ "${RUN_DP}" == "true" || "${RUN_DP}" == "1" ]]; then
  echo "[PIPELINE] 1/3 dialogue persona extraction"
  OUTPUT_DIR="${DP_DIR}" RESUME="${DP_RESUME:-false}" "${SCRIPT_DIR}/extract_dp_llama.sh"
  for ((round = 1; round <= MAX_EXTRACT_RETRY_ROUNDS; round++)); do
    ...
    OUTPUT_DIR="${DP_DIR}" MODEL_FAMILY="${MODEL_FAMILY}" RETRY_ROUND="${round}" "${SCRIPT_DIR}/retry_extract_dp.sh"
  done
  merge_dialogue_final
fi

if [[ "${RUN_BP}" == "true" || "${RUN_BP}" == "1" ]]; then
  echo "[PIPELINE] 2/3 behavioral persona extraction"
  OUTPUT_DIR="${BP_DIR}" RESUME="${BP_RESUME:-false}" "${SCRIPT_DIR}/extract_bp_llama.sh"
  for ((round = 1; round <= MAX_EXTRACT_RETRY_ROUNDS; round++)); do
    failed_count="$(count_extract_pending "${BP_DIR}" behavior)"
    [[ "${failed_count}" == "0" ]] && break
    echo "[PIPELINE] behavior retry round ${round} | failed=${failed_count}"
    OUTPUT_DIR="${BP_DIR}" MODEL_FAMILY="${MODEL_FAMILY}" RETRY_ROUND="${round}" "${SCRIPT_DIR}/retry_extract_bp.sh"
  done
  merge_behavior_final
fi

if [[ "${RUN_UPDATE}" == "true" || "${RUN_UPDATE}" == "1" ]]; then
  echo "[PIPELINE] 3/3 persona update with retry/apply/resume loop"
  for ((cycle = 1; cycle <= MAX_UPDATE_CYCLES; cycle++)); do
    ...
    OUTPUT_DIR="${UPD_DIR}" \
    BEHAVIOR_PATH="${BP_DIR}/extracted_behavior_only.llama.final.jsonl" \
    DIALOGUE_PATH="${DP_DIR}/extracted_dialogue_only.llama.final.jsonl" \
    RESUME=true \
    "${SCRIPT_DIR}/update_llama.sh"
    ...
    CUDA_VISIBLE_DEVICES="${UPDATE_CUDA_VISIBLE_DEVICES}" OUTPUT_DIR="${UPD_DIR}" RETRY_ROUND="auto" "${SCRIPT_DIR}/update_llama_retry.sh"
    python "${PERSONA_DIR}/tools/rebuild_update_resume_shards.py" \
      ...
      --merged-name "updated_personas_logs.llama.all.jsonl" \
      --from-merged \
      --no-backup
  done
fi
