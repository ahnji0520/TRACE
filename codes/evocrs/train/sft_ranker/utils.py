...
def get_checkpoint_model(base_model_path, checkpoint_path, thinking=False):
    tokenizer = _get_tokenizer(checkpoint_path)
...
def get_train_model(path, task, lora_path=None, thinking=False):

    tokenizer = _get_tokenizer(path)

    is_response_gen = 'response_generation' in task

    is_ranking = 'ranking' in task
    ...
    modules_to_save = ["embed_tokens", "lm_head"] if is_response_gen else None
...
def get_inference_model(path, thinking=False):
    tokenizer = _get_tokenizer(path)
